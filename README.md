...
> 一套可迁移、渐进式、基于 Claude Code 子智能体的通用开发 AI 协作工作流（描述示例是基于一个Godot+Python游戏）。

## 这是什么

这个框架解决一个核心问题：**AI 协作项目时，上下文容易被污染、多维度审查串行等待、项目状态分散不可追踪**。

它把 Claude Code 的子智能体（Subagent）上下文隔离和并行能力，与一套轻量但完整的文档/技能/规则体系结合，形成可复制的游戏开发协作工作流。( 暂时非ClaudeCode也可以使用会失去一些进阶功能)，该工作流不是空想的而是在工作过程中持续迭代而成
